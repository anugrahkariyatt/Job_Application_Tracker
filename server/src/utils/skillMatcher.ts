...
export function calculateRealSkillMatch(candidate: any, job: any): {
  ...
  if (!jobSkills.length) {
    ...
    return {
      ...
      missingSkills: [],
      strengths: [
        candidateSkills.length ? `Candidate possesses ${candidateSkills.length} verified technical skills` : "Standard profile submitted",
      ...
      summary: `Dynamic assessment: Candidate submitted ${candidateSkills.length} skills. Job requirements do not specify tech skills.`
    };
  }
