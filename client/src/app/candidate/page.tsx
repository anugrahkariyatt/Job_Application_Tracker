...
const timelineIconMap: Record<TimelineEvent['type'], { icon: React.ElementType; className: string }> = {
  applied: { icon: FileText, className: 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400' },
  review: { icon: Eye, className: 'bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400' },
  interview: { icon: Video, className: 'bg-teal-50 text-teal-600 dark:bg-teal-950 dark:text-teal-400' },
  offer: { icon: Star, className: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400' },
  rejected: { icon: XCircle, className: 'bg-rose-50 text-rose-600 dark:bg-rose-950 dark:text-rose-400' },
};

const notificationIconMap = {
  application: { icon: FileText, className: 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400' },
  job_alert: { icon: Bell, className: 'bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400' },
  company: { icon: TrendingUp, className: 'bg-violet-50 text-violet-600 dark:bg-violet-950 dark:text-violet-400' },
  interview: { icon: Video, className: 'bg-teal-50 text-teal-600 dark:bg-teal-950 dark:text-teal-400' },
};
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (appsResponse.data?.success && Array.isArray(appsResponse.data.data)) {
          const mappedApps: Application[] = appsResponse.data.data.map(mapApplicationToFrontend);
          ...
          const interviewApps = mappedApps.filter((a: Application) => a.status === 'Interview');
          const mockInterviews: Interview[] = interviewApps.map((a: Application, idx: number) => ({
            id: `iv-${idx}`,
            jobTitle: a.job.title,
            company: a.job.company,
            companyLogo: a.job.companyLogo,
            date: new Date(Date.now() + (idx + 1) * 2 * 24 * 60 * 60 * 1000).toISOString(),
            time: '10:00 AM',
            format: 'Video',
        ...
        if (jobsResponse.data?.success && Array.isArray(jobsResponse.data.data)) {
        ...
        if (notifResponse.data?.success && Array.isArray(notifResponse.data.data)) {
          const mappedNotifs = notifResponse.data.data.map(mapNotificationToFrontend);
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="p-8 text-center space-y-6 max-w-xl mx-auto">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-extrabold tracking-tight">Create Candidate Profile</h1>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Welcome to Limenzy, <span className="font-semibold text-foreground">{user?.name}</span>! Before you can search and apply to jobs, you need to create your candidate profile. Tell employers about your skills and experience!
              </p>
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardContent className="flex items-center gap-4 p-6">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Welcome back, {(profile?.fullName || user?.name || "Candidate").split(' ')[0]}</h1>
              <p className="mt-1 text-sm text-muted-foreground">{profile?.headline || 'Job Seeker'}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-3">
              <Progress value={profile?.profileCompleted ? 100 : 50} className="h-2" />
              <span className="text-sm font-semibold">{profile?.profileCompleted ? 100 : 50}%</span>
            </div>
          ...
          <Card className="cursor-pointer transition-shadow hover:shadow-md h-full">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Search new opportunities</p>
              </div>
            </CardContent>
          </Card>
        </Link>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <StatCard label="Applications Sent" value={stats.applicationsSent} icon={FileText} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">Latest Job Recommendations</h2>
            <Button variant="outline" size="sm" asChild>
                ...
                upcomingInterviews.map((iv) => (
                  <div key={iv.id} className="flex items-start gap-3">
                    <Avatar className="h-10 w-10 rounded-lg">
                      ...
                      <AvatarFallback className="rounded-lg">{iv.company.slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      ...
                      <Badge variant="secondary" className="mt-2 font-normal">{iv.format}</Badge>
                    </div>
                  </div>
                ))
              )}
            ...
            <CardContent className="space-y-3">
              {notifications.length === 0 ? (
                ...
                notifications.slice(0, 4).map((n) => {
                  const config = notificationIconMap[n.type] || notificationIconMap.application;
                  return (
                    <div key={n.id} className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">{relativeTime(n.time)}</p>
                      </div>
          ...
          {timeline.length === 0 ? (
            ...
            <div className="space-y-1">
              {timeline.map((event, idx) => {
                const config = timelineIconMap[event.type] || timelineIconMap.applied;
                return (
                  <div key={event.id} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={cn('flex h-9 w-9 items-center justify-center rounded-full', config.className)}>
                        <config.icon className="h-4 w-4" />
                      </div>
